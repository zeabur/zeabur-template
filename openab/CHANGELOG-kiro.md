...
- 檔名移除預留的 template code（尚未上傳至 Zeabur 官方 marketplace）
...
- 此模板使用 `ghcr.io/openabdev/openab` 映像（非 `openab-kiro`），以 `kiro-cli` 作為 agent 後端
- `config.toml` 僅於首次啟動由內建範本生成；升級後若要啟用 `allow_bot_messages` 等新選項，請直接編輯 `/home/agent/.config/openab/config.toml`

## [2026-04-12] - 初始版本

- 新增 OpenAB Kiro 模板（Discord → Kiro CLI 透過 ACP 橋接）
- 使用 `ghcr.io/openabdev/openab` 基底映像，預設 agent 後端為 `kiro-cli`
